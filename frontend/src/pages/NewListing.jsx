...
export default function NewListing() {
  ...
  const validateForm = () => {
    ...
    if (!listing.category) newErrors.category = "Select a category";
  ...
  const getInputClasses = (fieldName) => {
    const baseClasses =
      "border p-4 rounded outline-none transition-all duration-300 w-full";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.dismiss(loadingToast);
      toast.error(
        "Failed to add destination. Please login to add new destionation.",
      );
    }
  };
    ...
    <div className="max-w-2xl mx-auto p-8 bg-white border rounded shadow-md my-10">
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          <select
            className={getInputClasses("category") + " bg-white"}
