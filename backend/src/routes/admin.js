...
router.get("/stats", isLoggedIn, isAdmin, async (req, res) => {
  try {
    ...
    res.json({
      listings: { total: tL, pending: pL },
      reviews: { total: tR, pending: pR },
    });
  } catch (e) {
...
router.get("/pending", isLoggedIn, isAdmin, async (req, res) => {
  const pendingListings = await Listing.find({ status: "pending" }).populate(
    "owner",
  );
  res.json(pendingListings);
});

router.patch("/approve-all", isLoggedIn, isAdmin, async (req, res) => {
  await Listing.updateMany({ status: "pending" }, { status: "approved" });
  res.json({ success: true, message: "All Pending Listings Approved!" });
});

router.patch("/approve/:id", isLoggedIn, isAdmin, async (req, res) => {
  await Listing.findByIdAndUpdate(req.params.id, { status: "approved" });
  res.json({ success: true, message: "Listing Approved!" });
});

router.delete("/reject/:id", isLoggedIn, isAdmin, async (req, res) => {
  await Listing.findByIdAndDelete(req.params.id);
  res.json({ success: true, message: "Listing Rejected & Removed." });
});

router.get("/pending-reviews", isLoggedIn, isAdmin, async (req, res) => {
  const pendingReviews = await Review.find({ status: "pending" }).populate(
    "author",
  );
  res.json(pendingReviews);
});

router.patch("/reviews/approve/:id", isLoggedIn, isAdmin, async (req, res) => {
  await Review.findByIdAndUpdate(req.params.id, { status: "approved" });
  res.json({ success: true, message: "Review Approved!" });
});

router.delete("/reviews/reject/:id", isLoggedIn, isAdmin, async (req, res) => {
